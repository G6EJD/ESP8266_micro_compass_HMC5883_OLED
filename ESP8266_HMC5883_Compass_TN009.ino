...
void loop() {
 ...
 //Read data from each axis, 2 registers per axis
  ...
  // At my location it is :  -1° 20' W, or -1.33 Degrees, which needs to be in radians so = 1.33 / 180 * PI = 0.023 West is + E is -
  // Make declination = 0 if you can't find your Declination value, the error is negible for nearly all locations
  float declination = 0.023;
  ...
  angle = int(heading * 180/M_PI-180); // Convert radians to degrees for more a more usual result
  // For the screen -X = up and +X = down and -Y = left and +Y = right, so does not follow coordinate conventions
  dx = (0.7*radius * cos((angle-180)*3.14/180)) + centreX;  // calculate X position for the screen coordinates - can be confusing!
  dy = (0.7*radius * sin((angle-180)*3.14/180)) + centreY;  // calculate Y position for the screen coordinates - can be confusing!
  ...
  if (angle < 90) angle = angle + 270; else angle = angle - 90;
  display.print(angle);
  display.display();
  delay(100);
}
...
void Draw_Compass_Rose() {
  ...
  for (float i = 0; i <360; i = i + 22.5) {
    dxo = radius * sin((i-270)*3.14/180);
    dyo = radius * cos((i-270)*3.14/180);
    dxi = dxo * 0.95;
    dyi = dyo * 0.95;
